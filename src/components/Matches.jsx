        ...
        <>
          <div className="match-score-row">
            <div className={`match-team ${winner === match.team1 ? 'team-winner' : 'team-loser'}`}>
              <div className="team-players">{match.team1.join(', ')}{match.hasExtraPlayer && match.extraInTeam === 1 ? ' +⚡' : ''}</div>
            </div>
            ...
            <div className={`match-team team-right ${winner === match.team2 ? 'team-winner' : 'team-loser'}`}>
              <div className="team-players">{match.team2.join(', ')}{match.hasExtraPlayer && match.extraInTeam === 2 ? ' +⚡' : ''}</div>
            </div>
          </div>
          <div className="match-details-row">
            {match.mvp1 && <span className="detail-tag">⭐ {match.goals1 > match.goals2 ? match.mvp1 : match.mvp2}</span>}
          </div>
        </>
      ) : (
        <div className="match-pending-info"><p>Equipos asignados — resultado pendiente</p></div>
      )}
      <button className="btn-edit-match" onClick={e => { e.stopPropagation(); onEdit(match) }}>✏️</button>
    </div>
  )
}

export default function Matches({ matches, onEdit, onView, onNew }) {
